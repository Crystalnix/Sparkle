...
// The simplest way to disable original implementation and use our own while
// simplifying merges with the latest changes from Sparkle is to use #if.
#if 0
...
#else
// This is the first implementation of logs from Sparkle. They are now using
// the implementation above but we found that it would be hard for users to
// send us logs in case of any troubles, so we decided to revert the changes.
